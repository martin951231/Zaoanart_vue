    ...
    <div style="height:100%">
        ...
        <div class="lanmu clearfix">
            ...
            <div style="display: inline-block;float: right;">
                ...
                <div v-if="(user_name.is_attention_user==2)" @click="add_attention_user(user_name.uid)" style="float:left;cursor: pointer;color:#000;border:1px solid #000;margin:80px 10px 0px 10px;font-size: 13px;padding: 2px 10px;background-color: #fff;">+关注</div>
                <div style="width:100px;height:100px;overflow: hidden;border-radius: 50%;border: 1px solid #ccc;">
                ...
                <template v-if="(keep[0].img_ratio == 1)">
                    <div>
                        <a :href="'/#/keep/keepimg?id='+keep[0].keep_id" target="_blank">
                            <div class="keep gallery1" :id="keep[0].keep_id">
                                <div id="gallery_id">
    ...
    export default {
        ...
        methods:{
            initData(){
                ...
                this.$http.get(this.GLOBAL.baseurl + 'v1/home/finduserkeep',{params:{tel:this.telphone,uid: arr2['id']}}).then((response)=>{
            ...
            del_attention(keep_id){
                ...
                vm.$http.get(vm.GLOBAL.baseurl + 'v1/home/del_attention_keep',{params:{keep_id:keep_id,tel: vm.telphone}}).then((response)=>{
                    if(response.data == 1){
                        toastr.info("取消关注成功")
                        vm.$http.get(vm.GLOBAL.baseurl + 'v1/home/finduserkeep',{params:{tel:vm.telphone,uid: vm.info.id}}).then((response)=>{
            ...
            del_attention_user(attention_uid){
                ...
                vm.$http.get(vm.GLOBAL.baseurl + 'v1/home/del_attention_user',{params:{attention_uid:attention_uid,tel: vm.telphone}}).then((response)=>{
                    if(response.data == 1){
                        toastr.info("取消关注成功")
                        vm.$http.get(vm.GLOBAL.baseurl + 'v1/home/getusername1',{params:{tel:vm.telphone,uid: vm.info.id}}).then((response)=>{
                            vm.user_name = response.data
                            console.log(response.data)
                        })
                    }else if(response.data == 2){
                        toastr.warning("取消关注失败")
